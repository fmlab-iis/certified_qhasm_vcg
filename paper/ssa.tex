    ...
    \Match{$s$}
      \Case{$v \leftarrow e$}
        \Return $\langle \theta\mymapsto{v}{\theta(v) + 1}$, 
                $v^{\theta(v) + 1} \leftarrow$ \Call{SSAE}{$\theta$, $e$}$\rangle$
      \EndCase
      \Case{$\concat{v_h}{v_l} \leftarrow \dslcode{Split}(e, n)$}
        \State{\Return $\langle \theta\mymapsto{v_h,
            v_l}{\theta(v_h) + 1, \theta(v_l) + 1}$,
                $\concat{v_h^{\theta(v_h) + 1}}{v_l^{\theta(v_l) + 1}} \leftarrow
